...
public class CallActivity extends Activity implements AppRTCClient.SignalingEvents,
  ...
  //라이브 정보 받아오기
  private String host = "", title = "", tag = "", type2 = "";
        ...
        if (task.isSuccessful()) {
          ...
          if (document != null) {
            if (document.exists()) {
              ...
              title=document.getData().get("_live_title").toString();
              tag=document.getData().get("_live_tag").toString();
            } else {
  ...
  private void RecreateLive() {
    final String live_title = title;
    final String live_host = host;
    Random rnd = new Random();
    final int live_id = rnd.nextInt(152238) + 1000000;
    final String live_tag = tag;
    final String live_guest = "";

    Toast.makeText(getApplicationContext(), "라이브를 재생성합니다.", Toast.LENGTH_SHORT).show();
